...
$BackupPath = "F:\Dropbox\Documents\Backup"
. $DropBoxPath\Invoke-Parallel.ps1
Import-Module -Name F:\DropBox\Tag-It.psm1

Write-Output "Gathering Files..."
$FILESPACE = Get-ChildItem -Recurse  $BackupPath
Write-Output "Starting to tag."

Invoke-Parallel -InputObject $FILESPACE -Throttle 1 -LogFile F:\DropBox\ResultsBackup.txt -ImportModules -ImportVariables -ScriptBlock {
    cd $DropBoxPath
    if($_ -is [System.IO.FileInfo]) {
        ...
        $TAGS = ""
        while($SPLITDIR.Get($DIRCOUNT - 1) -ne "Backup" -or $DIRCOUNT -le 3) {
